...
# Dirs under KBOT_INSTALLER that are not workspace members (e.g. 3rdparty, generated "installer" tree).
_INSTALLER_WORKSPACE_EXCLUDE = frozenset(
    {
        "3rdparty",
        "installer",
    }
...
def _discover_installer_uv_package_subdirs(installer_root):
    """First-level subdirs under ``installer_root`` that contain a ``pyproject.toml`` (except excluded)."""
    names_out = []
    try:
        names = sorted(os.listdir(installer_root))
    except OSError:
        return names_out
    for name in names:
        if name.startswith(".") or name in _INSTALLER_WORKSPACE_EXCLUDE:
            continue
        path = os.path.join(installer_root, name)
...
def _write_installer_workspace_pyproject_stub(installer_root):
    """Write a minimal root ``pyproject.toml`` if missing (``uv add`` owns workspace metadata)."""
    out = os.path.join(installer_root, "pyproject.toml")
    if os.path.isfile(out):
        return
    req = _requires_python_spec_for_workspace()
    data = {
        "project": {
            "name": "konverso-installer-workspace",
            "version": "0.0.0",
            "description": "Installer workspace root next to cloned product repos.",
            "requires-python": req,
            "dependencies": [],
        },
    }
    ...
    with open(out, "w", encoding="utf-8") as f:
        f.write("# Generated by setup_workarea.py — uv workspace at KBOT_INSTALLER.\n")
        f.write(text)
...
class Installer:
    ...
    def Run(self):
        ...
        self._SetupInstallerUvWorkspace()
...
<p> Python Docs are only available with the "python-dev" solution installed.</p>
    ...
    def _SetupInstallerUvWorkspace(self):
        ...
        if not root or not os.path.isdir(root):
            return
        _write_installer_workspace_pyproject_stub(root)
        if not shutil.which("uv"):
            return
        subdirs = _discover_installer_uv_package_subdirs(root)
        if not subdirs:
            return
        cmd = (
            ["uv", "add"]
        ...
        if proc.returncode != 0:
            print(
                "uv add --workspace failed in KBOT_INSTALLER=%s (exit %s)."
