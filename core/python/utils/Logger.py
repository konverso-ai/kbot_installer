...
class KbotLogger(logging.Logger):
    ...
    def buildHandler(self, level, path=None):
        ...
        if os.path.basename(sys.argv[0]).startswith('GetProduct'):
            ...
            self.addHandler(cast(logging.Handler, DataDogHandler(level, path)))
            self.addHandler(cast(logging.Handler, KbotHandler(level, path)))
        else:
...
class KbotPackageLogger:
    ...
    def log(self, error: "ErrorCode", message=None, *args, exc_info=None, level=None, **kwargs):
    ...
    def log_and_raise(self, error: "ErrorCode", message=None, *args, exc_info=None, level=None, **kwargs):
