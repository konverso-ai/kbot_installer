...
class AzureBlob(BucketStorage):
    ...
    def __init__(self, account_url: str | None = None, container_name: str | None = None) -> None:
        ...
        if not self.container_name:
            log.debug("Missing storage account name for AzureBlob storage. Cannot be used. We recommend adding 'storage_account_container_name = %s",
                       self.container_name)
        self.container_client = None
        log.debug("Creating AzureBlob(account_url='%s', container_name='%s')", self.account_url, self.container_name)
    ...
    def __connect_to_blob_service(self) -> BlobServiceClient | None:
        ...
        if not self.account_url:
            log.warning("Azure Blob storage for account url '%s' is configured.", self.account_url)
            return None
        try:
            ...
            log.info("Successfully connected to Azure Blob storage for account name: %s", self.account_url)
            return blob_service_client
        except ResourceNotFoundError:
            log.warning("Azure Blob storage for account url '%s' does not exist.", self.account_url)
            return None
        except Exception as e:
            log.error("Failed to connect to Azure Storage account. Error: %s", e, exc_info=True)
            return None
    ...
    def create_container(self, blob_service_client: BlobServiceClient) -> None:
        ...
        try:
            ...
            log.error("Couldn't create Azure Blob Container %s due to %s", self.container_name, str(e))
    ...
    def set_container_name(self, container_name: str) -> None:
        ...
        if container_client is None:
            log.warning("Cannot update container information. Please review container name: %s", container_name)
            return
    ...
    def _get_container_client(self, container_name: str):
        ...
        if service_client:
            try:
                ...
                log.info("Connected to Azure Blob Storage container '%s'.", container_name)
                return self.container_client
            except ResourceNotFoundError:
                log.warning("Container '%s' does not exist.", container_name)
                return None
            except Exception as e:
                log.error("Failed to connect to Container on Azure Blob. Error: %s", e, exc_info=True)
                return None
        log.error("Container client initialization failed.")
        return None

    def get_container_client(self) -> BlobServiceClient | None:
    ...
    def set(self, key: str, value: str | bytes | Any, encoding: str = "utf-8", raise_on_status=False) -> None:
        ...
        try:
            ...
            container_client.upload_blob(name=key,
                                         data=content,
                                         overwrite=True)
    ...
    def get(self, key: str, encoding: str = "utf-8") -> str | None:
        ...
        container_client = self.get_container_client()
        if not container_client:
            log.error("Container client unavailable. Retrieval aborted. '%s'", self.container_name)
            return None
        try:
            blob_client = container_client.get_blob_client(key)
            log.debug("CONTAINER = %s :: %s", key, self.container_name)
            data = blob_client.download_blob().readall()
            log.debug("Successfully retrieved object from Azure Blob Storage: %s; encoding: %s", key, encoding)
    ...
    def list_folders(self, path: str = "") -> Iterator[str]:
        ...
        container_client = self.get_container_client()
        if not container_client:
            log.error("Container client unavailable. Cannot list folders in path '%s'", path)
            return
        ...
        try:
            ...
            folder_count = 0
            blob_hierarchy = container_client.walk_blobs(name_starts_with=path, delimiter='/')
    ...
    def restore_soft_deleted_blob(self, key: str) -> bool:
        ...
        try:
            ...
            log.debug("Blob '%s' not found (may have been permanently deleted or never existed).", key)
            return False
        except Exception as e:
            log.debug("Failed to restore soft-deleted blob '%s': %s", key, e, exc_info=True)
            return False
    ...
    def check_authorization(self) -> None:
        ...
        try:
            ...
            if self.container_client is None:
                raise RuntimeError("Authentication failed. Ensure the SAS token, Authorization header and Container Information are correct.")

            self.container_client.upload_blob(name="temp_blob_for_checking", data=b"Hi", overwrite=True)
            self.container_client.delete_blob("temp_blob_for_checking")
            log.debug("Authorization check passed. Connected to Azure Blob Storage in duration %.3f(s)", time.time()-start_time)
        except ClientAuthenticationError as e:
            log.error("Authentication failed. Check the SAS token and Authorization header. Error: %s", e, exc_info=True)
            raise RuntimeError("Authentication failed. Ensure the SAS token and Authorization header are correct.") from e
        except Exception as e:
