"""Base model for HTTP authentication fields."""
...
class HttpAuthBase(BaseModel, httpx.Auth):
    """Shared authentication fields for header-based HTTP auth.

    Self-contained base for the ``auth.http`` package: it has no dependency
    on ``auth.ssh`` or any shared top-level ``auth`` module.
    """
