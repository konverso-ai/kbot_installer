...
class TestCredentialManager:
    ...
    @patch.dict(os.environ, {"NEXUS_USERNAME": "test_user", "NEXUS_PASSWORD": "test_pass"})
    ...
    @patch.dict(os.environ, {"NEXUS_USERNAME": "test_user", "NEXUS_PASSWORD": "test_pass"})
    @patch("git.provider.credential_manager.create_auth")
    def test_get_auth_for_provider_nexus_general_exception(self, mock_create_auth) -> None:
    ...
    @patch.dict(os.environ, {"SSH_AUTH_SOCK": "/tmp/ssh-agent"}, clear=True)
    @patch("git.provider.credential_manager.create_auth")
    def test_get_auth_for_provider_github_success(self, mock_create_auth, mock_home) -> None:
    ...
    @patch.dict(os.environ, {"SSH_AUTH_SOCK": "/tmp/ssh-agent"}, clear=True)
    @patch("git.provider.credential_manager.create_auth")
    def test_get_auth_for_provider_bitbucket_success(self, mock_create_auth, mock_home) -> None:
    ...
    @patch.dict(os.environ, {"SSH_AUTH_SOCK": "/tmp/ssh-agent"}, clear=True)
    @patch("git.provider.credential_manager.create_auth")
    def test_get_auth_for_provider_bitbucket_import_error(self, mock_create_auth) -> None:
    ...
    @patch("git.provider.credential_manager.log")
    def test_get_auth_for_provider_unknown_provider_warning_second_call(self, mock_log) -> None:
        ...
        with patch.object(manager, "has_credentials", return_value=True):
            with patch.object(ProvidersConfig, "get_provider_config", return_value=None):
                result = manager.get_auth_for_provider("unknown")
