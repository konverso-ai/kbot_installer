...
class TestVaultFactory:
    ...
    def test_create_vault_creates_correct_type(
        self, vault_type: str, expected_name: str
    ) -> None:
    ...
    def test_create_vault_calls_factory_method(
        self, mock_factory_method: MagicMock
    ) -> None:
        ...
        mock_factory_method.assert_called_once_with(
            "alias", "kbot_installer.core.vault"
        )
        assert result == mock_vault

    @patch("kbot_installer.core.vault.factory.factory_method")
    def test_create_vault_passes_kwargs_to_factory(
        self, mock_factory_method: MagicMock
    ) -> None:
    ...
    @pytest.mark.parametrize(
        ("vault_type", "test_key", "expected_value"),
        [
            ("alias", "any_key", "ALIAS"),
    ...
    def test_created_vaults_work_correctly(
        self, vault_type: str, test_key: str, expected_value: str | None
    ) -> None:
        ...
        if vault_type == "azure":
            ...
            with (
                patch(
                    "kbot_installer.core.vault.azure_vault.SecretClient"
